function [res_table, table1, table2] = benchmark_no_u(T, param, steady_state, node_number,sim_length,sim_num)
...
Model = [pk_ss,pi_ss,mean(mean(n_sim)),sd_c,sd_n,sd_k,sd_i,sd_y,sd_w,corr_yc_gx,corr_yn_gx,corr_yk_gx,corr_yi_gx,corr_yw_gx]';
Model_std = [std_pk, std_pi, std(mean(n_sim)), std_sdc, std_sdn, std_sdk, std_sdi, std_sdy, std_sdw, std(diag(corr(y_c(:,t), c_c(:,t)))), std(diag(corr(y_c(:,t), n_c(:,t))))...
    std(diag(corr(y_c(:,t), k_c(:,t)))), std(diag(corr(y_c(:,t), i_c(:,t)))), std(diag(corr(y_c(:,t), w_c(:,t))))]';
US_data = [10,0.2133,1/3,0.0085,0.0138,0.0070,0.0426,0.0117,0.0102,0.8581,0.8333,-0.21,0.9124,0.3483]';
Mean_data = [mean(mean(c_sim)),mean(mean(n_sim)), mean(mean(k_sim)), mean(mean(y_sim)), mean(mean(i_sim)), mean(mean(w_sim))]';
Interval = [int_c; int_n; int_k; int_y; int_i; int_w]';
Interval_high = Interval(2,:)';
Interval_low = Interval(1,:)';

Interval_model_up = Model+2*[std_pk, std_pi, std(mean(n_sim)), std_sdc, std_sdn, std_sdk, std_sdi, std_sdy, std_sdw, std(diag(corr(y_c(:,t), c_c(:,t)))), std(diag(corr(y_c(:,t), n_c(:,t))))...
    std(diag(corr(y_c(:,t), k_c(:,t)))), std(diag(corr(y_c(:,t), i_c(:,t)))), std(diag(corr(y_c(:,t), w_c(:,t))))]';

Interval_model_low = Model-2*[std_pk, std_pi, std(mean(n_sim)), std_sdc, std_sdn, std_sdk, std_sdi, std_sdy, std_sdw, std(diag(corr(y_c(:,t), c_c(:,t)))), std(diag(corr(y_c(:,t), n_c(:,t))))...
    std(diag(corr(y_c(:,t), k_c(:,t)))), std(diag(corr(y_c(:,t), i_c(:,t)))), std(diag(corr(y_c(:,t), w_c(:,t))))]';
